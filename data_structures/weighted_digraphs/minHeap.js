// Min-oriented binary heap, ordered by vertex distance. 
...
var protoMinHeap = {
	...
	swim: function(k) {
		...
		// while we're not at the root and k's parent's distance
		// is bigger than k's distance:
		// swap their values and have k point to the parent index
		while (k > 1 && storage[Math.floor(k/2)].distance > storage[k].distance) {
	...
	sink: function(k) {
		...
		while (2*k <= this.n) {
			...
			if (j < this.n && storage[j].distance > storage[j+1].distance) j++;
			// if k's distance is not greater than the smallest child distance, then break
			if ( !(storage[k].distance > storage[j].distance) ) break;
